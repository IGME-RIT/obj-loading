...
glm::vec2 InputManager::GetMouseCoords()
{
	...
	ret.x = (((float)_mousePos[0] / (float)_windowSize[0]) * 2.0f - 1.0f) * _aspectRatio * 3.14159f / 180.0f;
	ret.y = -(((float)_mousePos[1] / (float)_windowSize[1]) * 2.0f - 1.0f) * 3.14159f / 180.0f;
	return ret;
}
