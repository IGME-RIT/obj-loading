...
void ResourceManager::Init()
{
	...
	char fragmentShader[] = "../Assets/fshader.glsl";
	char vertexShader[] = "../Assets/vshader.glsl";
	char outputFS[] = "outColor";

	char SphereModel[] = "../Assets/Sphere.obj";
	char CubeModel[] = "../Assets/Cube.obj";
	char PlaneModel[] = "../Assets/Plane.obj";

	
	phongFragShader = CompileShader(fragmentShader, GL_FRAGMENT_SHADER);
	phongVertShader = CompileShader(vertexShader, GL_VERTEX_SHADER);

	shaders[0] = phongFragShader;
	shaders[1] = phongVertShader;

	phongShader = LinkShaderProgram(shaders, 2, 0, outputFS);
	...
	LoadOBJ(SphereModel, sphere, phongShader);
	LoadOBJ(CubeModel, cube, phongShader);
	LoadOBJ(PlaneModel, plane, phongShader);
}
