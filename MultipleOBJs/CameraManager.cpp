...
{
	_proj = glm::perspective(fov, aspectRatio, near, far);
...
void CameraManager::Update(float dt)
{
	...
	if (InputManager::cursorLocked())
	{
		...
		_position.x -= _position.x > 2*3.14159 ? 2 * 3.14159 : 0.0f;

		_position.y += mouseCoords.y * rotRate;
	}

	_position.y = _position.y > 0.15 * 3.14159 ? 0.15 * 3.14159 : _position.y;
	_position.y = _position.y < -0.15 * 3.14159 ? -0.15 * 3.14159 : _position.y;
	...
	_camPos = glm::vec4(0.0f, 0.0f, -20.0f, 1.0f) * rotMat;
